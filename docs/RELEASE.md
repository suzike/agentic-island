...
- 终端字符回显不会触发重复 React 状态，xterm/ConPTY 尺寸通知已去重；PowerShell 初始化在交互输入开放前完成；目录选择、工作区导入导出与 External Yield 回归通过；第一、第二终端的输入几何、命令执行、退出码、危险确认、恢复和工具分区通过隔离 Electron 审计。
