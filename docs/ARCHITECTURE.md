# Agentic-Island v0.6.1 架构说明

本文档描述当前 `0.6.1` 实现，以工作区源码和自动化测试为准。
...
`llm-proxy.ts` 同时支持 OpenAI 兼容 `/chat/completions` 与 Anthropic 原生 `/messages`。公共请求层负责多轮 history、快速/深度预算、多模态 parts、响应 reasoning 解析、Base URL 归一化和错误脱敏；支持发现模型的端点可通过 `/models` 同步当前账号实际目录。DeepSeek V4 的快/深模式分别关闭或开启 thinking，并在深度模式使用 max effort；Kimi Code 固定使用 `api.kimi.com/coding/v1` 与三个官方 Coding Model ID，K3 映射 low/max effort；Kimi 开放平台使用 `api.moonshot.cn/v1`，两类密钥互不复用。Claude 官方端点使用 `x-api-key`、`anthropic-version` 和 Messages 图片格式，当前 Sonnet/Opus 深度模式使用 adaptive thinking。外网请求经 Electron `net.fetch`，继承 Windows 系统代理。
...
`kb.ts` 支持文件夹、文件、网页和岛内问答文本；读取源码/文本、PDF 与 DOCX，或直接接收回答、完整分支和框选片段，分块后调用 embedding 模型并持久化向量。同一会话来源使用稳定 key 覆盖更新，检索结果带来源、路径、片段和相似度，问答层强制组合引用。
...
`npm test` 当前顺序执行 33 个离线 `scripts/test-*.ts`，排除必须依赖真实 Claude 登录的 `test-real-claude.ts`。模型设置测试覆盖旧配置迁移、供应商密钥隔离、Kimi 双端点和请求参数兼容；录屏测试覆盖几何合成、帧预算、暂停时钟、字幕、FFmpeg 参数、真实 FFmpeg 导出、分片会话与工程迁移。直接被 raw Node 加载的主进程模块不得在顶层运行时导入 Electron，也不得使用 strip-types 不支持的 TypeScript enum 或参数属性。
