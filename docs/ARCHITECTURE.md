# Agentic-Island v0.6.6 架构说明

本文档描述当前 `0.6.6` 实现，以工作区源码和自动化测试为准。
...
![问答会话增强管线](conversation-pipeline.svg)

### 云模型代理

`llm-proxy.ts` 同时支持 OpenAI 兼容 `/chat/completions` 与 Anthropic 原生 `/messages`。公共请求层负责多轮 history、快速/深度预算、多模态 parts、响应 reasoning 解析、Base URL 归一化和错误脱敏；支持发现模型的端点可通过 `/models` 同步当前账号实际目录。`logic/providers.ts` 以供应商、模型、归一化 Base URL 和 API Key 作为完整连接身份，同模型不同账号可并存，并由问答页和设置页共享原子切换状态机。DeepSeek V4 的快/深模式分别关闭或开启 thinking；Kimi Code 固定使用 `api.kimi.com/coding/v1` 与三个官方 Coding Model ID，K3 映射 low/max effort；Kimi 开放平台使用 `api.moonshot.cn/v1`，两类密钥互不复用。OpenAI 当前目录为 GPT-5.6 Sol/Terra/Luna，使用 `max_completion_tokens` 与 reasoning effort。Claude 当前目录为 Sonnet 5、Opus 4.8、Fable 5 和 Haiku 4.5，按型号映射 adaptive thinking、effort、思考摘要或手动预算。外网请求经 Electron `net.fetch`，继承 Windows 系统代理。

问答会话使用稳定分支 ID 和父子关系。Fork 复制截止节点以前的消息并继承分支记忆/指令；切换时当前分支整体归档，合并时由模型先把来源分支压缩为长期上下文。删除父分支时子分支重新挂到上一级，避免失效谱系。`historyFromThread` 排除显式忽略项，始终保留钉选消息，并把受预算限制的文本附件和分支记忆注入后续轮次；代理层不再二次截断已选定的置顶上下文。`logic/methodologies.ts` 维护 15 种单轮回答方法、23 种气泡分析方法、分类元数据、模型提示词和本地关键词推荐；单轮方法统一注入云模型、本机 Agent 与知识库 RAG，发送后从 composer 清除。本机 Agent 通过 `buildAgentContextPrompt` 显式接收同一套岛内历史，不使用 Claude CLI 的全局最近会话。每条回答的继续追问保存在该消息的 `followups` 子线程中，只使用主线程截止该回答的历史和本支线历史；回答分析保存在 `analyses`，多模型候选保存在 `variants`，二者均附着原回答且默认不进入主会话历史。主回答、追问、分析、多模型、记忆压缩和分支合并均按稳定分支 ID 回写；异步检索使用发起分支的规则快照，同一分支一次只允许一个回答生成任务。持久化前递归剔除 `typing/live`，再由 `compactChatMessages` 在 60 条上限内优先保留重要消息和最近消息。
...
`kb.ts` 支持文件夹、文件、网页和岛内问答文本；读取源码/文本、PDF 与 DOCX，或直接接收回答、完整分支和框选片段，分块后调用 embedding 模型并持久化向量。同一会话来源使用稳定 key 覆盖更新，检索结果带来源、路径、片段和相似度，问答层强制组合引用。Embedding 的 Base URL、模型和 API Key 作为独立配置持久化；旧版 `embedModel` 会在首次升级时复制原聊天连接，此后聊天供应商切换不会改变索引、检索或写入端点。
...
- PowerShell 的 UTF-8 与 OSC 提示符初始化作为 `-NoExit -Command` 启动参数执行，在交互提示符与 xterm 输入通道可用前完成；新建第二个终端时不再通过 PTY 写入初始化命令。
- 新建、切换终端标签或重新挂载 xterm 时，终端组件先向主面板发送键盘交互保持信号，清除尚未执行的自动收起计时器，再迁移焦点；显式 Esc 收起不会被该信号重新唤出。
