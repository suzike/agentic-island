# Agentic-Island v0.6.0 架构说明

本文档描述当前 `0.6.0` 实现，以工作区源码和自动化测试为准。
...
`llm-proxy.ts` 调用 OpenAI 兼容 `/chat/completions`，支持多轮 history、快速/深度 token 预算、`reasoning_content` 和多模态 parts；同一代理也通过 `/models` 同步当前账号实际可用的模型目录。渲染层按供应商保存独立的模型、Base URL 与 API Key，并对旧版单配置状态做一次性迁移，避免切换 DeepSeek、Kimi 等供应商时串用密钥。外网请求经 Electron `net.fetch`，继承 Windows 系统代理。

问答会话使用稳定分支 ID 和父子关系。Fork 复制截止节点以前的消息并继承分支记忆/指令；切换时当前分支整体归档，合并时由模型先把来源分支压缩为长期上下文。`historyFromThread` 排除显式忽略项，始终保留钉选消息，并把文本附件和分支记忆注入后续轮次。多模型会诊使用已保存且彼此隔离的供应商配置并行请求，候选回答保存在同一消息中；共识和辩论模式再由当前主模型主持收敛。
...
`npm test` 当前顺序执行 32 个离线 `scripts/test-*.ts`，排除必须依赖真实 Claude 登录的 `test-real-claude.ts`。模型设置测试覆盖旧配置迁移、供应商密钥隔离和目录升级；录屏测试覆盖几何合成、帧预算、暂停时钟、字幕、FFmpeg 参数、真实 FFmpeg 导出、分片会话与工程迁移。直接被 raw Node 加载的主进程模块不得在顶层运行时导入 Electron，也不得使用 strip-types 不支持的 TypeScript enum 或参数属性。
