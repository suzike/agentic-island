# Agentic-Island v0.6.5 架构说明

本文档描述当前 `0.6.5` 实现，以工作区源码和自动化测试为准。
...
问答会话使用稳定分支 ID 和父子关系。Fork 复制截止节点以前的消息并继承分支记忆/指令；切换时当前分支整体归档，合并时由模型先把来源分支压缩为长期上下文。删除父分支时子分支重新挂到上一级，避免失效谱系。`historyFromThread` 排除显式忽略项，始终保留钉选消息，并把受预算限制的文本附件和分支记忆注入后续轮次；代理层不再二次截断已选定的置顶上下文。本机 Agent 通过 `buildAgentContextPrompt` 显式接收同一套岛内历史，不使用 Claude CLI 的全局最近会话。每条回答的继续追问保存在该消息的 `followups` 子线程中，只使用主线程截止该回答的历史和本支线历史；回答分析保存在 `analyses`，多模型候选保存在 `variants`，二者均附着原回答且默认不进入主会话历史。主回答、追问、分析、多模型、记忆压缩和分支合并均按稳定分支 ID 回写；异步检索使用发起分支的规则快照，同一分支一次只允许一个回答生成任务。持久化前递归剔除 `typing/live`，再由 `compactChatMessages` 在 60 条上限内优先保留重要消息和最近消息。
