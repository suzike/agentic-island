...
const PRESETS: { label: string; t: Tokens }[] = [
  ...
  // 复古风·高级感配色：奶油底 + 双强调色，主色明度已压到 WCAG AA（按钮对比度 ≥ 4.5）
  { label: '奶霜', t: preset({ th: '221', th2: '57', ths: '84', cs: '.71', css: '2', mode: 'light', bg: '.9', ga: '.9', fi: '1.18', bl: '40', sh: '.55', c1: '.114', c2: '.18', sc: '.042', l1: '.48', l2: '.75', tx: '.16', gr: '100' }) },
  { label: '月苍', t: preset({ th: '176', th2: '146', ths: '99', cs: '.27', css: '1.15', mode: 'light', bg: '.9', ga: '.9', fi: '1.18', bl: '42', sh: '.48', c1: '.043', c2: '.06', sc: '.023', l1: '.38', l2: '.73', tx: '.16', gr: '150' }) },
  { label: '牙色', t: preset({ th: '246', th2: '28', ths: '91', cs: '.49', css: '2', mode: 'light', bg: '.88', ga: '.9', fi: '1.22', bl: '38', sh: '.58', c1: '.078', c2: '.162', sc: '.062', l1: '.38', l2: '.51', tx: '.16', gr: '130' }) },
  { label: '御茶', t: preset({ th: '199', th2: '267', ths: '112', cs: '.35', css: '1.9', mode: 'light', bg: '.9', ga: '.92', fi: '1.1', bl: '44', sh: '.45', c1: '.056', c2: '.027', sc: '.038', l1: '.49', l2: '.38', tx: '.16', gr: '160' }) },
  { label: '肌色', t: preset({ th: '250', th2: '84', ths: '71', cs: '.79', css: '2', mode: 'light', bg: '.9', ga: '.9', fi: '1.18', bl: '40', sh: '.5', c1: '.127', c2: '.166', sc: '.049', l1: '.45', l2: '.81', tx: '.16', gr: '120' }) }
]
...
function ControlPreview({ mode }: { mode: ThemeMode }): React.JSX.Element {
  ...
  return <div style={previewShell}>
    ...
    <div style={{ ...row, marginTop: 7 }}><span style={icon}><Volume2 size={12} /></span><div style={{ flex: 1 }}><div style={{ ...text.dim(), color: ink(1), fontWeight: 650 }}>声音</div><div style={{ height: 5, borderRadius: R.pill, marginTop: 5, background: fill(4), overflow: 'hidden' }}><div style={{ width: '63%', height: '100%', background: accent(.78) }} /></div></div><BatteryCharging size={13} color={accentText()} /></div>
  </div>
}
...
function TerminalPreview(): React.JSX.Element {
  return <div style={{ ...previewShell, display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
    ...
    <div style={{ flex: 1, padding: 13, background: 'oklch(.095 .015 var(--ths))', color: 'oklch(.92 .01 var(--th))', font: "11px/1.75 'Cascadia Code', monospace", ...({ '--accent-text-max-l': 1, '--sem-text-max-l': 1 } as React.CSSProperties) }}><div><span style={{ color: accentText() }}>PS</span> npm run typecheck</div><div style={{ color: sem.calm }}>✓ renderer types passed</div><div style={{ color: sem.calm }}>✓ node types passed</div><div style={{ color: 'oklch(.72 .02 var(--th))' }}>ready in 1.84s</div><div><span style={{ color: accent2() }}>PS</span> <span style={{ opacity: .7 }}>_</span></div></div>
  </div>
}
