...
function TerminalPreview(): React.JSX.Element {
  return <div style={{ ...previewShell, display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
    ...
    <div style={{ flex: 1, padding: 13, background: 'oklch(.095 .015 var(--ths))', color: 'oklch(.92 .01 var(--th))', font: "11px/1.75 'Cascadia Code', monospace", ...({ '--accent-text-max-l': 1 } as React.CSSProperties) }}><div><span style={{ color: accentText() }}>PS</span> npm run typecheck</div><div style={{ color: sem.calm }}>✓ renderer types passed</div><div style={{ color: sem.calm }}>✓ node types passed</div><div style={{ color: 'oklch(.72 .02 var(--th))' }}>ready in 1.84s</div><div><span style={{ color: accent2() }}>PS</span> <span style={{ opacity: .7 }}>_</span></div></div>
  </div>
}
