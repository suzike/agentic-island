...
[![Release v0.6.1](https://img.shields.io/badge/release-v0.6.1-e89a2e)](https://github.com/suzike/agentic-island/releases/tag/v0.6.1)
...
<img src="screenshots/ask-v061.png" alt="Agentic-Island v0.6.1 · 会话增强问答工作台真实截图" width="880"/>
...
- **会话分支工作流**：可从任意提问或回答 Fork，保留父子谱系、独立命名和分支记忆；支持快速切换，以及把其他分支经 AI 压缩后合并到当前长期上下文。
- **可控上下文**：分支级长期记忆和持续指令真实注入模型；消息可钉选为永久上下文或排除，文本附件跨轮次持续生效，并显示上下文条目、字符、估算 token 与预算占用。
- **多模型协作**：从已保存的跨供应商模型中选择 2–4 个并行作答，支持并列比较、共识提炼和分歧辩论；候选答案保存在同一消息内，可采用为后续主上下文。
- **回答推进工具**：围绕现有回答继续做自检、假设与风险、替代路径、问题拆解、苏格拉底式澄清、知识库核验和下一问建议，不改写原消息。
- **明确且可靠的会话交互**：每条回答下的“继续追问”仍在原气泡内形成独立支线；分析和多模型结论固定附着到被处理的回答，不混入主会话。生成期间锁定同一分支的重复发送，异步结果始终回到发起分支；新会话、Fork 和删除在执行前说明结果。
- **对话知识沉淀**：完整分支、单条回答或框选片段可直接写入本地向量知识库，并作为 `conversation` 来源立即参与 RAG 检索。
- **模型协议可靠性**：DeepSeek V4 的快速/深度模式真实映射 thinking 开关与 effort；Kimi Code 会员服务与开放平台独立配置，K3 映射 low/max 思考强度；Claude 直连原生 Anthropic Messages API，并支持模型发现、图片输入和 adaptive thinking。
- **配置迁移与诊断**：每家供应商独立保存模型、Base URL 与 API Key；Kimi 固定官方目录、Claude 旧模型 ID 和误填完整 endpoint 自动修复。认证/套餐错误给出可操作提示，所有上游错误和异常统一脱敏。
- **异步状态一致性**：模型测试/目录同步只回写发起时的配置；知识库检索、回答分析和分支操作始终使用发起分支的规则，避免切换会话后串用上下文。
- **动态灵感同源**：问答空态推荐与常驻迷你条共用启用主题、AI、GitHub 和工作简报内容池，按轮播节奏更新，避免固定示例长期不变。
- **既有能力完整保留**：Agent 审批、项目工作台、快捷编排、待办、便签、资讯、复盘、终端、截图与专业录屏能力均未删除。
...
<table>
<tr>
<td width="50%" align="center"><img src="screenshots/ask-v061.png" alt="问答工作台"/><br/><b>问答</b><br/><sub>会话分支 · 可控上下文 · 多模型协作 · 本地 RAG</sub></td>
<td width="50%" align="center"><img src="screenshots/shortcuts-v061.png" alt="快捷工程工作流"/><br/><b>快捷</b><br/><sub>项目上下文 · 12 条工程工作流</sub></td>
</tr>
<tr>
<td width="50%" align="center"><img src="screenshots/todos-v061.png" alt="智能待办工作台"/><br/><b>待办</b><br/><sub>计划 · 看板 · 任务属性 · AI 执行辅助</sub></td>
<td width="50%" align="center"><img src="screenshots/notes-v061.png" alt="灵感便签知识工作台"/><br/><b>灵感便签</b><br/><sub>Markdown · 双链 · 模板 · 知识工具</sub></td>
</tr>
<tr>
<td width="50%" align="center"><img src="screenshots/news-v061.png" alt="资讯情报工作台"/><br/><b>资讯</b><br/><sub>观察清单 · 信号处置 · 情报雷达</sub></td>
<td width="50%" align="center"><img src="screenshots/review-v061.png" alt="每日复盘与工作洞察"/><br/><b>复盘</b><br/><sub>活动流水 · 日报周报 · 效率洞察</sub></td>
</tr>
<tr>
<td width="50%" align="center"><img src="screenshots/recording-v061.png" alt="专业录屏工坊"/><br/><b>录屏</b><br/><sub>多源采集 · 实时运镜 · 三轨剪辑 · AI 后期</sub></td>
<td width="50%" align="center"><img src="screenshots/settings-v061.png" alt="设置与主题系统"/><br/><b>设置</b><br/><sub>接入诊断 · 多显示器 · 主题 · 自动化</sub></td>
</tr>
</table>
...
| **问答** | OpenAI 兼容云模型、Anthropic Messages API 与本机 Claude Code/Codex；快速/深度模式；会话分支树与任意节点 Fork；分支切换、命名和合并；长期记忆与持续会话指令；上下文钉选/排除和附件连续上下文；多模型并行、共识与辩论；附着原回答的 AI 分析；每条回答气泡内的连续追问支线；引用追问；对话写入知识库；动态灵感推荐 |
...
| **本地知识库** | 接入文件夹、文件、网页和问答会话；支持常见源码/文本、PDF、DOCX；回答、完整分支或框选片段可直接沉淀；分块、向量索引、引用问答、Wiki 概览与重建索引 |
...
活动分支、归档分支、长期记忆和持续指令分别持久化。每次主会话请求统一经过上下文组装器，再进入云模型、本机 Agent 或多模型讨论。本机 Claude/Codex 显式接收当前岛内分支上下文，不依赖 CLI 的全局最近会话。回答下的继续追问保存在该气泡的 `followups` 支线中；分析、候选回答与多模型汇总附着到目标回答，不会自动进入主会话历史。所有异步结果按稳定分支 ID 回写，持久化前清除临时生成状态。回答、完整分支和框选片段可通过主进程写入本地向量知识库。
...
在「设置」中可配置 OpenAI 兼容 `/chat/completions` 端点，也可选择 `Claude` 直连 Anthropic 原生 `/v1/messages`。Kimi For Coding 会员密钥请选择 `Kimi Code`（`https://api.kimi.com/coding/v1`，固定 `kimi-for-coding`、`kimi-for-coding-highspeed`、`k3`）；Moonshot AI 开放平台密钥请选择 `Kimi 开放平台`（`https://api.moonshot.cn/v1`）。两类密钥互不通用，模型、Base URL 和 API Key 分别保存。支持发现模型的供应商可通过 `/models` 获取账号实际可用目录；误填完整 Chat Completions/Messages 地址会自动归一为 Base URL。问答、待办 AI、便签、资讯、复盘、截图分析和主题设计共用当前主模型配置；语义检索和本地知识库可单独设置 embedding 模型。API Key 经 Electron `safeStorage` 使用 Windows DPAPI 加密。
...
`npm test` 当前顺序执行 33 个离线脚本，自动排除 `test-real-claude.ts`。覆盖 Agent 生命周期、审批闭环、Codex 跟随、外部让位、日历、待办、快捷、终端、Markdown、便签、资讯、复盘、供应商模型配置迁移、DeepSeek/Kimi/Anthropic 请求协议、录屏合成/会话/工程/FFmpeg 导出、截图轮询、主题、向量、知识链接、番茄钟、SRS、工程计算和工作台迁移。真实 Claude CLI 与云供应商账号连通测试需在已登录或持有有效密钥的环境中单独运行。
