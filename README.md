...
[![Release v0.6.0](https://img.shields.io/badge/release-v0.6.0-e89a2e)](https://github.com/suzike/agentic-island/releases/tag/v0.6.0)
...
<img src="screenshots/ask-v060.png" alt="Agentic-Island v0.6.0 · 会话增强问答工作台真实截图" width="880"/>
...
- **模型配置可靠性**：DeepSeek、Kimi 等供应商分别持有模型、Base URL 和 API Key；支持旧配置迁移、在线 `/models` 同步，并对上游认证错误做密钥脱敏。
...
<table>
<tr>
<td width="50%" align="center"><img src="screenshots/ask-v060.png" alt="问答工作台"/><br/><b>问答</b><br/><sub>会话分支 · 可控上下文 · 多模型协作 · 本地 RAG</sub></td>
<td width="50%" align="center"><img src="screenshots/shortcuts-v060.png" alt="快捷工程工作流"/><br/><b>快捷</b><br/><sub>项目上下文 · 12 条工程工作流</sub></td>
</tr>
<tr>
<td width="50%" align="center"><img src="screenshots/todos-v060.png" alt="智能待办工作台"/><br/><b>待办</b><br/><sub>计划 · 看板 · 任务属性 · AI 执行辅助</sub></td>
<td width="50%" align="center"><img src="screenshots/notes-v060.png" alt="灵感便签知识工作台"/><br/><b>灵感便签</b><br/><sub>Markdown · 双链 · 模板 · 知识工具</sub></td>
</tr>
<tr>
<td width="50%" align="center"><img src="screenshots/news-v060.png" alt="资讯情报工作台"/><br/><b>资讯</b><br/><sub>观察清单 · 信号处置 · 情报雷达</sub></td>
<td width="50%" align="center"><img src="screenshots/review-v060.png" alt="每日复盘与工作洞察"/><br/><b>复盘</b><br/><sub>活动流水 · 日报周报 · 效率洞察</sub></td>
</tr>
<tr>
<td width="50%" align="center"><img src="screenshots/recording-v060.png" alt="专业录屏工坊"/><br/><b>录屏</b><br/><sub>多源采集 · 实时运镜 · 三轨剪辑 · AI 后期</sub></td>
<td width="50%" align="center"><img src="screenshots/settings-v060.png" alt="设置与主题系统"/><br/><b>设置</b><br/><sub>接入诊断 · 多显示器 · 主题 · 自动化</sub></td>
</tr>
</table>
...
| **问答** | OpenAI 兼容云模型与本机 Claude Code/Codex；快速/深度模式；会话分支树与任意节点 Fork；分支切换、命名和合并；长期记忆与持续会话指令；上下文钉选/排除和附件连续上下文；多模型并行、共识与辩论；AI 自检/假设/替代路径/问题拆解/知识核验/下一问；引用与气泡内追问；对话一键写入知识库；动态灵感推荐 |
...
活动分支、归档分支、长期记忆和持续指令分别持久化。每次请求统一经过上下文组装器，再进入主模型或多模型会诊；回答、完整分支和框选片段可通过主进程写入本地向量知识库。
...
在「设置」中配置任一 OpenAI 兼容 `/chat/completions` 端点。DeepSeek、Kimi 等供应商分别保存模型、Base URL 和 API Key，切换时不会串用；“同步可用模型”通过当前端点的 `/models` 获取账号实际可用目录。问答、待办 AI、便签、资讯、复盘、截图分析和主题设计共用当前主模型配置；语义检索和本地知识库可单独设置 embedding 模型。API Key 经 Electron `safeStorage` 使用 Windows DPAPI 加密。
...
`npm test` 当前顺序执行 32 个离线脚本，自动排除 `test-real-claude.ts`。覆盖 Agent 生命周期、审批闭环、Codex 跟随、外部让位、日历、待办、快捷、终端、Markdown、便签、资讯、复盘、供应商模型配置迁移、录屏合成/会话/工程/FFmpeg 导出、截图轮询、主题、向量、知识链接、番茄钟、SRS、工程计算和工作台迁移。真实 Claude CLI 集成测试需在已登录环境中单独运行。
