...
<img src="screenshots/news-v030.png" alt="Agentic-Island v0.3 资讯情报工作台真实截图" width="880"/>
...
- **前端视觉体系重建**：新建 `src/renderer/src/ui/` 设计系统（令牌 / 共享组件 / 动效预设 / 语义图标），11 个分区与全部浮层统一到新基座，emoji 图标全面替换为 lucide。
- **全面 Apple（macOS/iOS）化**：层级从描边制改为填充制（`fill` 阶梯 + `hairline` 发型线）；iOS inset grouped 分组列表、滑动 thumb 分段控件、白钮开关、SF 排版（负字距标题 / label 四级墨色）；按压反馈改为 iOS 式透明度下沉。
- **材质与动效进阶**：面板极光氛围光 + 玻璃颗粒噪点 + 主题色环境投影；Tab 栏分区图标 + layoutId 滑动胶囊；浮层统一 overlayPop 弹出；动效引入 framer-motion。
- **多显示器定位全面修复**：显示器热插拔 / 分辨率 / DPI 变化自动重定位；跨 DPI 屏定位校验重试；全屏模式真正铺满物理显示器（含任务栏区域）；设置页展示真实显示器列表。
- 问答思考过程改为显式可折叠区块；修复分区切换动画卡顿（移除 blur 入场与串行退出等待）。
...
<table>
<tr>
<td width="50%" align="center"><img src="screenshots/ask-v030.png" alt="问答工作台"/><br/><b>问答</b><br/>云模型、本地 Agent、知识库与引用追问</td>
<td width="50%" align="center"><img src="screenshots/shortcuts-v030.png" alt="快捷工程工作流"/><br/><b>快捷</b><br/>项目上下文与 12 条工程工作流</td>
</tr>
<tr>
<td width="50%" align="center"><img src="screenshots/todos-v030.png" alt="智能待办工作台"/><br/><b>待办</b><br/>计划、看板、任务属性与 AI 执行辅助</td>
<td width="50%" align="center"><img src="screenshots/notes-v030.png" alt="灵感便签知识工作台"/><br/><b>灵感便签</b><br/>Markdown、双链、模板和知识管理工具</td>
</tr>
<tr>
<td width="50%" align="center"><img src="screenshots/news-v030.png" alt="资讯情报工作台"/><br/><b>资讯</b><br/>观察清单、信号处置与情报雷达</td>
<td width="50%" align="center"><img src="screenshots/review-v030.png" alt="每日复盘与工作洞察"/><br/><b>复盘</b><br/>活动流水、日报周报、效率洞察与成长记录</td>
</tr>
</table>
...
渲染层全部视觉收敛在 `src/renderer/src/ui/`：

- `tokens.ts`：Apple 填充制层级——`fill(1-4)` 填充阶梯、`hairline()` 发型线分隔、Apple 圆角阶梯、iOS label 四级墨色、SF 排版（负字距标题）；颜色全部消费 OKLCH 主题变量（`--th/--th2/--ths/--cs/--css/--pl`），6 套内置主题与自定义主题自动跟随。
- `components.tsx`：Button / IconButton / Card / Chip / Badge / Input / Segmented（iOS 滑动 thumb）/ SectionHeader / EmptyState / Switch（iOS 白钮）/ Slider / Group（inset grouped 列表）。
- `motion.ts`：framer-motion 动效预设（入场 / 浮层 / 列表 stagger / iOS 透明度按压）。
...
```text
Agentic-Island-Setup-0.3.0.exe
```
